...
此模式用于观察单次模型请求占用了多少上下文窗口。例如最近一次请求使用了 20,000 token，而模型上下文窗口为 200,000，屏幕会显示约 `10%`。它适合判断当前对话距离上下文上限还有多远。
...
此模式用于统计整个 Codex 会话的累计 token 消耗，并与自定义参考预算比较。例如累计使用了 500,000 token、预算为 2,000,000，屏幕会显示 `25%`。`--session-budget` 只是本地显示使用的参考线，不代表 OpenAI 的实际额度或限制。
